...
from schema import LocationRequest, MinistrySearchRequest, UpdateMinistryRequest, UpdateMemberRequest
...
def get_location(request: LocationRequest, db: Session= Depends(get_db)):
    try:
        ...
        stmt1= (select(mla.c.name, mla.c.party, mla.c.criminal_cases, mla.c.education, mla.c.photo_url, mla.c.slap_count, mla.c.rose_count, mla.c.constituency, mla.c.constituency_key)
        ...
        stmt2= (select(mp.c.name, mp.c.party, mp.c.criminal_cases, mp.c.education, mp.c.photo_url, mp.c.slap_count, mp.c.rose_count, mp.c.constituency, mp.c.constituency_key)
...
def get_minister(request: MinistrySearchRequest, db: Session= Depends(get_db)):
    try:
        ...
        stmt= (select(minister.c.ministry, minister.c.minister_name, minister.c.party, minister.c.photo_url, minister.c.slap_count, minister.c.rose_count, minister.c.manifesto_points).where(minister.c.minister_name==minister_name))
        final_minister_details= db.execute(stmt).mappings().first()
...
def update_ministry_count(request: UpdateMinistryRequest, db: Session= Depends(get_db)):
    try:
        ...
        member= Table("ministers", metadata, autoload_with= engine)
